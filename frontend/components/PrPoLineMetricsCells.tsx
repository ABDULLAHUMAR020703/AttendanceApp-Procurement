...
export function PrPoLineMetricsCells({ summary }: { summary: PoLineSummary | null | undefined }) {
  ...
  const tone = summary.exceeds_po_limit ? 'text-rose-200' : 'text-emerald-100/95';
  const border = summary.exceeds_po_limit ? 'border-l-2 border-rose-500' : 'border-l-2 border-emerald-600/50';

  return (
    <>
      <TD className={`text-xs ${tone} ${border} pl-2`}>{summary.item_code ?? '—'}</TD>
      <TD className={`text-xs max-w-[160px] truncate ${tone}`}>
        <span title={desc}>{desc}</span>
      </TD>
      <TD className={`text-xs ${tone}`}>
        {summary.unit_price != null ? formatPkr(summary.unit_price) : '—'}
      </TD>
      <TD className={`text-xs ${tone}`}>{qtyLabel(summary.requested_quantity)}</TD>
      <TD className={`text-xs ${tone}`}>{formatPkr(summary.requested_amount)}</TD>
      <TD className={`text-xs ${tone}`}>
        <span title={summary.remaining_tooltip}>{formatPkr(summary.remaining_amount)}</span>
      </TD>
      <TD className={`text-xs font-medium ${tone}`}>
        <span className="inline-flex flex-col gap-0.5">
          <span>{formatPkr(summary.remaining_after_approval)}</span>
          {summary.exceeds_po_limit ? (
            <span className="text-[10px] uppercase tracking-wide text-rose-300">Exceeds PO limit</span>
          ) : null}
        </span>
      </TD>
    </>
